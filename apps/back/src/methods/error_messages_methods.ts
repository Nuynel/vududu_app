...
export enum ERROR_NAME {
  ...
  USER_NOT_FOUND = '400_USER_NOT_FOUND',
...
export const errorHandler = (res: Response, e: Error) => {
  if (e instanceof CustomError) {
    ...
    switch (e.type) {
      ...
      case ERROR_NAME.USER_NOT_FOUND:
      case ERROR_NAME.WRONG_EMAIL:
        return res.status(400).send({message: e.type})
  ...
  if (e instanceof MongoError) {
    ...
    return res.status(500).send({message: ERROR_NAME.INTERNAL_SERVER_ERROR});
  }
